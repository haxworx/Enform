   ...
   for (; elem; elem = snd_mixer_elem_next(elem))
     {
        ...
        snd_mixer_selem_get_id(elem, sid);
        name = snd_mixer_selem_id_get_name(sid);
        if (name && (!strcmp(name, "Master") || !strcmp(name, "PCM"))) {
            return eina_stringshare_add(name);
	}
     }
                          ...
                          int right)
{
   ...
   if (is_raspberrypi) {
	double better_range = max - min;
        double volume = left;
	if (right > left)
		volume = right;
	double perc = better_range / 100.0;
	long new_val = (double) round(min + (perc * volume));
	snd_mixer_selem_set_playback_volume_all(channel, new_val);
	return 1;
   }
                         ...
                         E_Mixer_Channel_State *state)
{
   ...
   if (!state)
    ...
    const char *default_alsa_card = "hw:0";
    const char *card_name = e_mixer_system_get_card_name(default_alsa_card);

    /* This hack is for the Raspberry Pi (3) */
    if (card_name && !strncmp(card_name, "bcm2835", 7) && snd_mixer_selem_is_playback_mono(channel)) {
        is_raspberrypi = 1;
    }

    if (card_name) eina_stringshare_del(card_name);
