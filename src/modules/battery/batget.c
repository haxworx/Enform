...
static void
openbsd_acpi_init(void)
{
   /* nothing to do here */ 
}

static void
openbsd_acpi_check(void)
{
   time_left = -1;
   battery_full = -1;
   ...
   if (sysctl(ac_mib, 5, &s, &slen, NULL, 0) != -1)
      have_power = (int) s.value;
   ...
   if (current_charge == 0 || last_full_charge == 0)
     ...
     if (current_charge >= last_full_charge)
        percent = 100; // workaround

     battery_full = (int) percent;
}
   ...
   if (bats)
     {
        ...
        if (powers)
		  ...
		  snprintf(buf, sizeof(buf), "%s%s", info->path, "/state");
        ...
        EINA_ITERATOR_FOREACH(bats, info)
	     ...
	     snprintf(buf, sizeof(buf), "%s%s", info->path, "/info");
	     /* BOGO
             strcpy(buf, info->path);
             strcat(buf, "/info");
	     */
