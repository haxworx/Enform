...
static void _ibox_icon_fill_window_content(IBox_Icon *ic);
static void _ibox_icon_fill_icon(IBox_Icon *ic);
...
Eina_Bool _window_refresh(void *data)
{
   IBox *b = data;
   Eina_List *l;
   IBox_Icon *ic;
   ...
   EINA_LIST_FOREACH(b->icons, l, ic)
     {
   	...
   	ic->o_icon = NULL;
   	ic->o_icon2 = NULL;
        _ibox_icon_fill_window_content(ic);
     }
...
_gc_init(E_Gadcon *gc, const char *name, const char *id, const char *style)
{
   ...
   //ecore_timer_add(0.1, _window_refresh, b);
...
static void _ibox_icon_fill_window_content(IBox_Icon *ic)
{
   ...
   visual = ecore_x_default_visual_get(display, screen);
   colormap = ecore_x_default_colormap_get(display, screen);

   E_Border *bd = ic->border;
 
   xwin = ic->border->bg_win;

   /* This is the window */
   Ecore_X_Window_State *state = NULL;
   unsigned int num = 0;

   ecore_x_netwm_window_state_get(ic->border->client.win, &state, &num);
   ...
   ecore_x_window_attributes_get(xwin, &watt);

   img = ecore_x_image_new(sw, sh, visual, ecore_x_window_depth_get(xwin)); // watt.depth);
   if (!img) return;

   Eina_Bool success = ecore_x_image_get(img, xwin, 0, 0, x, y, sw, sh);
   if (!success) 
   {
      /* The whole window is not rendered.
         Try to get what is available to us.
      */
 
      Ecore_X_Window root = bd->zone->container->manager->root;
      xwin = bd->client.win;
      while (xwin != root)
        {
      ...
      success = ecore_x_window_attributes_get(xwin, &watt);
      if (!success) goto safe_out;
      visual = watt.visual;
      success = ecore_x_image_get(img, xwin, x, y, 0, 0, sw, sh);
      if (!success) goto safe_out;
    }
   ...
   dest = malloc(sw * sh * sizeof(int)); 
   if (!dest) return;

   success = ecore_x_image_to_argb_convert(src, bpp, bpl, colormap,
	visual, 0, 0, sw, sh, dest, (sw * sizeof(int)),0, 0);
   if (!success) {
     goto safe_out;
   }
     
   Evas *evas = evas_object_evas_get(ic->ibox->o_box); 
   Evas_Object *o = evas_object_image_filled_add(evas); 

   evas_object_image_size_set(o, sw, sh);

   evas_object_image_data_set(o, (void *) dest) ;
   evas_object_image_data_update_add(o, 0, 0, sw, sh);
  
   edje_object_part_swallow(ic->o_holder, "e.swallow.content", o);
   evas_object_pass_events_set(o, 1);
   evas_object_show(o);

   ic->o_icon = o;
   
   Evas *evas2 = evas_object_evas_get(ic->ibox->o_box);
   Evas_Object *o2 = evas_object_image_filled_add(evas2);

   evas_object_image_size_set(o2, sw, sh);

   evas_object_image_data_set(o2, (void *) dest) ;

   evas_object_image_data_update_add(o2, 0, 0, sw, sh);

   edje_object_part_swallow(ic->o_holder2, "e.swallow.content", o2 );
   evas_object_pass_events_set(o2, 1);
   evas_object_show(o2);

   ic->o_icon2 = o2;
   _ibox_icon_fill_label(ic);
   if (img) ecore_x_image_free(img);
   return;
...
{
  /*
   E_Module *m = e_module_find("comp");
   if (!m ||  !m->enabled) // !ecore_x_composite_query() || !e_config->use_composite) 
     {
	return;
     }
   else
